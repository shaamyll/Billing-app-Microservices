...
const adapter = new PrismaPg({ connectionString: env.PAYMENT_DATABASE_URL, ssl: false });
...
export const connectPrisma = async (): Promise<boolean> => {
  try {
    ...
    logger.info("✅ Prisma connected (payment-service)");
    return true;
  } catch (err: unknown) {
...
export const closePrisma = async (): Promise<void> => {
  try {
    ...
    logger.info("🛑 Prisma disconnected (payment-service)");
  } catch (err: unknown) {
