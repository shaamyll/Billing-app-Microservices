...
const adapter = new PrismaPg({ connectionString: env.TRIP_DATABASE_URL, ssl: false });
...
export const connectPrisma = async (): Promise<boolean> => {
  try {
    ...
    logger.info("✅ Prisma connected (trip-service)");
    return true;
  } catch (err: unknown) {
...
export const closePrisma = async (): Promise<void> => {
  try {
    ...
    logger.info("🛑 Prisma disconnected (trip-service)");
  } catch (err: unknown) {
