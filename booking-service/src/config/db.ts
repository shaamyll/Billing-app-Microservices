...
const adapter = new PrismaPg({ connectionString: env.BOOKING_DATABASE_URL, ssl: false });
...
export const connectPrisma = async (): Promise<boolean> => {
  try {
    ...
    logger.info("✅ Prisma connected (booking-service)");
    return true;
  } catch (err: unknown) {
...
export const closePrisma = async (): Promise<void> => {
  try {
    ...
    logger.info("🛑 Prisma disconnected (booking-service)");
  } catch (err: unknown) {
